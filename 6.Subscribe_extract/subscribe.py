...
#기본 DB불러오기
total = pd.read_csv("total.csv", index_col=0) #경로/이름 설정
subscribe = pd.read_csv("subscribe_2023-06-22.csv") #경로/이름 설정
...
for index, row in subscribe.iterrows():
    ...
    if srchwrd and srchwrd.strip():
        ...
        if len(DT_Query2) > 0: #예외처리 (검색한 대상이 있을/없을)
